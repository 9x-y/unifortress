...
pub const XTS_KEY_SIZE: usize = AES256_KEY_SIZE * 2; // XTS использует два ключа AES
...
pub fn derive_key(password: &[u8], salt: &[u8]) -> Result<Vec<u8>> {
    ...
    let params = Params::new(ARGON_M_COST, ARGON_T_COST, ARGON_P_COST, Some(DERIVED_KEY_SIZE))
...
impl VolumeHeader {
    /// Создает новый экземпляр заголовка с уникальным UUID и сгенерированной солью.
    /// HMAC инициализируется нулями и должен быть рассчитан и установлен позже.
    pub fn new() -> Self {
        let salt = generate_salt();
        let uuid = Uuid::new_v4().into_bytes(); // Генерируем UUID v4
        ...
        VolumeHeader {
            ...
            header_hmac: [0u8; HEADER_HMAC_SIZE], // Инициализируем нулями
        }
    }
...
/// * `sector_data` - Данные сектора для шифрования (должны быть >= 16 байт).
///
/// # Returns
